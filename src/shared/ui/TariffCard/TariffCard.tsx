...
export const TariffCard: FC<TariffCardProps> = ({ priceName = '-', price = 5887, numberTickets = 0 }) => {
    ...
    return (
        <div className={styles.priceCard} style={priceName === 'Эконом Плюс' ? { background: '#C2DCFF' } : { background: '#EBF3FF' }}>
            <div className={styles.priceblock} >
                <h4 className={styles.titlePriceCard} >{priceName}<img className={styles.group3__} alt={`Иконка ${priceName}`} src={getGroupImage()} /></h4>
                <h2 className={styles.price}>{formatPrice(price)} ₽</h2>
            </div>
            {(numberTickets <= 10) ? <div className={styles.numberOfTickets} ><img src={warning} />Осталось {numberTickets}</div> : null}
        </div >
    )
}