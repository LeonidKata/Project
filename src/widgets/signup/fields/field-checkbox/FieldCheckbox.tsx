...
export const CheckboxConfirm = forwardRef<HTMLInputElement, IField>(
  ({ defaultChecked, type = 'checkbox', onChange, value, errors }, ref) => {
    return (
        ...
        <label htmlFor="checkbox">
          ...
          {errors?.isAgree && (
            <div className={clsx(styles['wrapper-error'])}>
              ...
              <p className={styles.error}>{errors?.isAgree?.message}</p>
            </div>
          )}
        </label>
        Я прочитал(-а){' '}
        <a className={clsx(styles['confirm-text'])} href="#0">
          условия пользовательского соглашения и согласен (-на) с ними
        </a>
      </div>
    );
  },
