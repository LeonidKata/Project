...
export const FieldAnswer = forwardRef<HTMLInputElement, IField>(
  ({ errors, placeholder, required, title, validAnswer, type = 'text', ...rest }, ref) => {
    return (
      <div className={clsx(styles['wrapper-input'])}>
        <label className={clsx(styles['label-input'])}>
          ...
          <input
            className={clsx(
              styles['input-field'],
              styles[errors?.answer && 'input-field-error'],
              styles[validAnswer && 'input-field-valid'],
            )}
